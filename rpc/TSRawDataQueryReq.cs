...
public partial class TSRawDataQueryReq : TBase
{
  ...
  {
    var tmp238 = new TSRawDataQueryReq();
    tmp238.SessionId = this.SessionId;
    if((Paths != null))
    {
      tmp238.Paths = this.Paths.DeepCopy();
    }
    if(__isset.fetchSize)
    {
      tmp238.FetchSize = this.FetchSize;
    }
    tmp238.__isset.fetchSize = this.__isset.fetchSize;
    tmp238.StartTime = this.StartTime;
    tmp238.EndTime = this.EndTime;
    tmp238.StatementId = this.StatementId;
    if(__isset.enableRedirectQuery)
    {
      tmp238.EnableRedirectQuery = this.EnableRedirectQuery;
    }
    tmp238.__isset.enableRedirectQuery = this.__isset.enableRedirectQuery;
    return tmp238;
  }
            ...
            {
              {
                ...
                Paths = new List<string>(_list239.Count);
                for(int _i240 = 0; _i240 < _list239.Count; ++_i240)
                {
                  string _elem241;
                  _elem241 = await iprot.ReadStringAsync(cancellationToken);
                  Paths.Add(_elem241);
                }
    ...
    try
    {
      ...
      if((Paths != null))
      {
        ...
        await oprot.WriteFieldBeginAsync(field, cancellationToken);
        {
          ...
          foreach (string _iter242 in Paths)
          {
