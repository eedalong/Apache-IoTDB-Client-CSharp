...
public partial class TSOpenSessionResp : TBase
{
  ...
  {
    var tmp43 = new TSOpenSessionResp();
    if((Status != null))
    {
      tmp43.Status = (TSStatus)this.Status.DeepCopy();
    }
    tmp43.ServerProtocolVersion = this.ServerProtocolVersion;
    if(__isset.sessionId)
    {
      tmp43.SessionId = this.SessionId;
    }
    tmp43.__isset.sessionId = this.__isset.sessionId;
    if((Configuration != null) && __isset.configuration)
    {
      tmp43.Configuration = this.Configuration.DeepCopy();
    }
    tmp43.__isset.configuration = this.__isset.configuration;
    return tmp43;
  }
      ...
      while (true)
      {
        ...
        switch (field.ID)
        {
          ...
          case 4:
            ...
            {
              {
                TMap _map44 = await iprot.ReadMapBeginAsync(cancellationToken);
                Configuration = new Dictionary<string, string>(_map44.Count);
                for(int _i45 = 0; _i45 < _map44.Count; ++_i45)
                {
                  string _key46;
                  string _val47;
                  _key46 = await iprot.ReadStringAsync(cancellationToken);
                  _val47 = await iprot.ReadStringAsync(cancellationToken);
                  Configuration[_key46] = _val47;
                }
    ...
    try
    {
      ...
      if((Configuration != null) && __isset.configuration)
      {
        ...
        await oprot.WriteFieldBeginAsync(field, cancellationToken);
        {
          ...
          foreach (string _iter48 in Configuration.Keys)
          {
            await oprot.WriteStringAsync(_iter48, cancellationToken);
            await oprot.WriteStringAsync(Configuration[_iter48], cancellationToken);
          }
